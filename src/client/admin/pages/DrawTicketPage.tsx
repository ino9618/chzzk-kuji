...
const initialItems: DraftItem[] = [
  { label: '스페셜 상품', description: '오늘의 특별 상품', imageUrl: null, weight: 20, quantity: 1 },
  { label: '일반 상품', description: '방송에서 안내할 상품', imageUrl: null, weight: 80, quantity: 3 },
];
...
export function DrawTicketPage({ refreshKey = 0 }: { refreshKey?: number }) {
  ...
  const [selectionMode, setSelectionMode] = useState<'quantity' | 'probability'>('quantity');
  ...
  const create = async () => {
    ...
    try {
      await api.createDrawTicket({ name, command, ticketPrice, selectionMode, items });
      await refresh();
      setFeedback({ ok: true, text: '새 뽑기권을 시작했습니다.' });
    } catch {
      setFeedback({ ok: false, text: '모든 항목을 확인해 주세요. 직접 확률은 합계가 정확히 100%여야 합니다.' });
  ...
  const activeWeight = activeItems.filter((item) => item.remainingQuantity > 0).reduce((sum, item) => sum + (state.session?.selectionMode === 'quantity' ? item.remainingQuantity : item.weight), 0);
  ...
  const draftQuantity = useMemo(() => items.reduce((sum, item) => sum + item.quantity, 0), [items]);
  const draftProbability = (item: DraftItem) => selectionMode === 'quantity'
    ? (draftQuantity > 0 ? item.quantity / draftQuantity * 100 : 0)
    : item.weight;
  const directProbabilityValid = selectionMode === 'quantity' || draftWeight === 100;
  ...
  return <div className="admin-page draw-ticket-page">
    ...
    {state.active && state.session ? <section className="draw-ticket-active roulette-config-panel">
      ...
      <div className="draw-ticket-item-table">
        ...
        {activeItems.map((item) => <div className={`draw-ticket-item-row ${item.remainingQuantity === 0 ? 'sold-out' : ''}`} key={item.id}><div className="draw-ticket-thumb">{item.imageUrl ? <img src={item.imageUrl} alt="" /> : <ImageIcon />}</div><div className="draw-ticket-item-copy"><strong>{item.label}</strong>{item.description && <small>{item.description}</small>}</div><span>{item.remainingQuantity > 0 && activeWeight > 0 ? `${Number(((state.session?.selectionMode === 'quantity' ? item.remainingQuantity : item.weight) / activeWeight * 100).toFixed(1))}%` : '-'}</span><span><b>{item.remainingQuantity}</b> / {item.totalQuantity}</span></div>)}
      </div>
      ...
      <div className="roulette-config-head"><div className="roulette-icon"><DrawTicketIcon /></div><div><strong>새 뽑기권 설정</strong><p>수량에 비례하거나 항목별 확률을 직접 지정할 수 있습니다.</p></div></div>
      ...
      <div className="draw-ticket-mode-picker"><span>확률 설정 방식</span><div className="segmented-control" role="group" aria-label="뽑기권 확률 설정 방식"><button className={selectionMode === 'quantity' ? 'active' : ''} onClick={() => setSelectionMode('quantity')}>수량 비례</button><button className={selectionMode === 'probability' ? 'active' : ''} onClick={() => setSelectionMode('probability')}>직접 확률</button></div><p>{selectionMode === 'quantity' ? '남은 수량이 많을수록 당첨 확률이 높아집니다.' : `항목별 확률 합계 ${draftWeight}% / 100%`}</p></div>
      <div className={`draw-ticket-item-table editable ${selectionMode === 'quantity' ? 'quantity-mode' : 'probability-mode'}`}><div className="draw-ticket-item-head"><span>사진</span><span>항목과 설명</span>{selectionMode === 'probability' && <span>당첨 확률</span>}<span>예상 확률</span><span>수량</span><span /></div>{items.map((item, index) => <div className="draw-ticket-item-row" key={index}><div className="draw-ticket-image-control">{item.imageUrl ? <img src={item.imageUrl} alt="" /> : <ImageIcon />}<label title="항목 사진 선택"><span className="sr-only">{index + 1}번 항목 사진 선택</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => { void selectImage(index, event.target.files?.[0]); event.target.value = ''; }} /></label>{item.imageUrl && <button aria-label={`${index + 1}번 항목 사진 삭제`} onClick={() => updateItem(index, { imageUrl: null })}>×</button>}</div><div className="draw-ticket-item-inputs"><input aria-label={`${index + 1}번 뽑기 항목`} value={item.label} maxLength={60} onChange={(event) => updateItem(index, { label: event.target.value })} placeholder="항목명" /><input aria-label={`${index + 1}번 뽑기 설명`} value={item.description} maxLength={160} onChange={(event) => updateItem(index, { description: event.target.value })} placeholder="시청자에게 보여줄 설명" /></div>{selectionMode === 'probability' && <NumberStepper aria-label={`${index + 1}번 당첨 확률`} min={1} max={100} suffix="%" value={item.weight} onValueChange={(weight) => updateItem(index, { weight })} />}<span>{`${Number(draftProbability(item).toFixed(1))}%`}</span><NumberStepper aria-label={`${index + 1}번 수량`} min={1} max={1000} value={item.quantity} onValueChange={(quantity) => updateItem(index, { quantity })} /><button className="icon-button" aria-label={`${index + 1}번 항목 삭제`} disabled={items.length <= 1} onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}><TrashIcon /></button></div>)}</div>
      ...
      {!directProbabilityValid && <InlineFeedback tone="error">직접 확률의 합계를 100%로 맞춰주세요.</InlineFeedback>}
      <div className="roulette-actions"><button disabled={pending || !directProbabilityValid} onClick={create}>{pending ? '시작 중' : '뽑기 시작'}</button></div>
    </section>}
