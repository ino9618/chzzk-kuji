...
export function BroadcastPreflightPage({ session, chzzkStatus, kujiEnabled }: { session: SessionState; chzzkStatus: string; kujiEnabled: boolean }) {
  ...
  return (
    <div className="admin-page preflight-page">
      ...
      <section className="workflow-section"><div className="workflow-heading"><div><h2>OBS 오버레이</h2><p>OBS 브라우저 소스와 동일한 화면을 새 창에서 확인합니다.</p></div><a className="button-link secondary-button" href="/overlay.html" target="_blank" rel="noreferrer">오버레이 미리보기</a></div></section>
    </div>
  );
}
