...
export function RoulettePage() {
  ...
  return <div className="admin-page roulette-page">
    ...
    <section className="roulette-config-panel">
      <div className="roulette-config-head"><div className="roulette-icon"><RouletteIcon /></div><div><strong>룰렛 조건</strong><p>한 번의 후원당 룰렛을 한 번 돌립니다.</p></div><label>최소 후원 금액<div className="input-suffix"><input type="number" min={1} value={config.minimumAmount} onChange={(event) => setConfig((current) => ({ ...current, minimumAmount: Number(event.target.value) }))} /><span>치즈</span></div></label></div>
      <div className="roulette-registration"><div><strong>시청자 항목 등록</strong><p>설정 금액 이상 후원하며 <code>!등록 항목명</code>을 입력하면 가중치 1의 항목으로 추가됩니다.</p></div><label>등록 후원 금액<div className="input-suffix"><input type="number" min={1} value={config.registrationAmount} onChange={(event) => setConfig((current) => ({ ...current, registrationAmount: Number(event.target.value) }))} /><span>치즈</span></div></label></div>
      <div className="roulette-item-table"><div className="roulette-item-head"><span>결과 항목</span><span>가중치</span><span>확률</span><span /></div>{config.items.map((item, index) => <div className="roulette-item-row" key={index}><input aria-label={`${index + 1}번 룰렛 항목`} value={item.label} maxLength={40} onChange={(event) => updateItem(index, 'label', event.target.value)} /><input aria-label={`${index + 1}번 룰렛 가중치`} type="number" min={1} max={1000} value={item.weight} onChange={(event) => updateItem(index, 'weight', Number(event.target.value))} /><span>{totalWeight > 0 ? `${Math.round(item.weight / totalWeight * 100)}%` : '0%'}</span><button className="icon-button" aria-label={`${index + 1}번 룰렛 항목 삭제`} disabled={config.items.length <= 2} onClick={() => setConfig((current) => ({ ...current, items: current.items.filter((_, itemIndex) => itemIndex !== index) }))}><TrashIcon /></button></div>)}</div>
