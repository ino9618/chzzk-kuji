...
export function App() {
  ...
  return (
    <AppShell page={page} onNavigate={setPage} status={chzzkStatus} onLogout={logout}>
      ...
      {page === 'overlay' && <OverlaySettingsPage session={session} nicknameMode={nicknameMode} onSetNicknameMode={async (mode) => { await api.setNicknameMode(mode); setNicknameMode(mode); setBasicSettings((current) => ({ ...current, nicknameMode: mode })); }} onTestOverlay={async (payload) => { const response = await socket.timeout(10_000).emitWithAck('overlay:test', payload); if (!response?.ok) throw new Error('overlay_test_failed'); return response; }} onTestRoulette={async (payload) => { const response = await socket.timeout(10_000).emitWithAck('overlay:roulette-test', payload); if (!response?.ok) throw new Error('roulette_test_failed'); return response; }} />}
