...
1. `http://localhost:5173/admin.html` 접속 → 비밀번호 로그인
2. 맨 위 "치지직 연결 상태"가 `connected`인지 확인 (다른 상태면 아래 "문제 생겼을 때" 참고)
3. "현재 회차" 칸에서:
...
OBS에서 소스 추가 → **브라우저** 선택 → URL에 `http://localhost:5173/overlay.html` 입력 → 배경 투명하게 번호판이 화면에 뜹니다. 판매된 번호는 초록색으로 강조되고, 방금 팔린 번호는 잠깐 반짝이는 효과가 나타납니다.
...
- **연결 상태가 `reconnecting`**: 일시적 문제, 잠시 후 자동으로 `connected`로 돌아옵니다.
- **화면에 빨간 배너("재인증 필요")가 뜨면**: 배너의 "치지직 재인증하기" 링크 클릭 → 인가 완료 후 **서버(터미널)를 껐다 다시 켜야** 실제로 연결이 복구됩니다.
- **비밀번호를 바꾸고 싶으면**: 관리자 화면 맨 아래 "비밀번호 변경" 섹션 이용 (자세한 내용은 아래 참고).

## 관리자 로그인 방법 (2가지)

- **네이버(치지직) 계정으로 로그인** (권장): 로그인 화면의 초록색 버튼 클릭 → 네이버 로그인/인가 → 자동으로 관리자 화면 진입.
  - **처음 한 번은 사용할 수 없습니다** — 먼저 비밀번호로 로그인해서 치지직 계정을 연결(`/api/chzzk/oauth/start`)해야, 그 계정이 이 보드의 "소유자"로 등록되어 이후 네이버 로그인이 가능해집니다.
  - 다른 사람이 자기 네이버 계정으로 로그인 시도하면 "이 채널의 관리자 계정이 아닙니다"로 거부됩니다.
  - 네이버 로그인에 성공할 때마다 저장된 치지직 토큰도 함께 갱신되어, 토큰 만료 걱정이 줄어듭니다.
- **비밀번호 로그인** (비상용): `.env`의 `ADMIN_PASSWORD`로 최초 설정된 비밀번호. OAuth에 문제가 생겨도 이 방법으로 항상 들어올 수 있습니다.

## 관리자 비밀번호 변경

1. 관리자 화면 맨 아래 "비밀번호 변경" 섹션에서 현재 비밀번호 → 새 비밀번호 → 새 비밀번호 확인 입력 후 "비밀번호 변경" 클릭
2. 성공하면 바로 적용됩니다 (재시작 불필요)
3. 새 비밀번호는 4자 이상이어야 하고, 잊어버리면 되돌릴 방법이 없으니(DB 초기화 필요) 꼭 기억해두세요
...
6. OBS 브라우저 소스 URL을 `https://xxxx.onrender.com/overlay.html`로 등록 (주소가 고정이라 한 번만 설정)
...
- [ ] OBS 브라우저 소스로 `/overlay.html` 추가 → 투명 배경과 실시간 하이라이트 애니메이션 확인
