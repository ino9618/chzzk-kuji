...
import { buildDrawTicketSpeech, buildRouletteSpeech, buildWinnerSpeech, synthesizeGoogleTts } from '../../src/server/googleTts';
...
describe('Google Cloud TTS', () => {
  it('builds Korean winner copy including anonymous fallback', () => {
    ...
    expect(buildDrawTicketSpeech({ nickname: '후원자', label: '디저트 쿠폰' })).toBe('후원자님의 뽑기권 결과는 디저트 쿠폰입니다. 축하합니다.');
  });
